...
#include "ui_allhouselist.h"
#include <qDebug>
#include <QSqlQuery>
allhouselist::allhouselist(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::allhouselist)
{
    ui->setupUi(this);
    ...
    if(query.exec())
    {
        qDebug()<<"allhouselist : sql select success";
        while (query.next()) {
...
QString allhouselist::GetFindKind(int kind)
{
    QString thekind = "";
    if(kind == 0)
    {
...
void allhouselist::on_pushButton_clicked()
{
    this->~allhouselist();
}
