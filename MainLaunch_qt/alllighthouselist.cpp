...
#include "ui_alllighthouselist.h"
#include <qDebug>
#include <QSqlQuery>
alllighthouselist::alllighthouselist(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::alllighthouselist)
{
    ui->setupUi(this);
    ...
    if(query.exec())
    {
        qDebug()<<"alllighthouselist : sql select success";
        while (query.next()) {
...
alllighthouselist::~alllighthouselist()
{
    delete ui;
}

QString alllighthouselist::GetFindKind(int kind)
{
    QString thekind = "";
    if(kind == 0)
    {
...
void alllighthouselist::on_pushButton_clicked()
{
    this->~alllighthouselist();
}
