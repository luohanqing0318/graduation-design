...
#include "ui_smallhouselist.h"
#include <qDebug>
#include <QSqlQuery>
smallhouselist::smallhouselist(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::smallhouselist)
{
    ui->setupUi(this);
    ...
    if(query.exec())
    {
        qDebug()<<"smallhouselist : sql select success";
        while (query.next()) {
...
smallhouselist::~smallhouselist()
{
    delete ui;
}

QString smallhouselist::GetFindKind(int kind)
{
    QString thekind = "";
    if(kind == 0)
    {
