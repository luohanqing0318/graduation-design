...
#include "ui_goodhouselist.h"
#include <qDebug>
#include <QSqlQuery>
goodhouselist::goodhouselist(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::goodhouselist)
{
    ui->setupUi(this);
    QString sql = "SELECT * FROM `rooms` where kind = 2;";
    QSqlQuery query;
    query.prepare(sql);

    if(query.exec())
    {
        qDebug()<<"goodhouselist : sql select success";
        while (query.next()) {
...
goodhouselist::~goodhouselist()
{
    delete ui;
}

QString goodhouselist::GetFindKind(int kind)
{
    QString thekind = "";
    if(kind == 0)
    {
