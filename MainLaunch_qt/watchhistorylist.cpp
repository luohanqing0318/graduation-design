...
#include "ui_watchhistorylist.h"
#include <qDebug>
#include <QSqlQuery>
#include "common_data.h"
watchhistorylist::watchhistorylist(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::watchhistorylist)
{
    ui->setupUi(this);
    ...
    for(int i = 0; i< temp.size(); i++)
    {
        ...
        query_select.bindValue(":a2", temp[i]);
        if(query_select.exec())
        {
            qDebug()<<"watchhistorylist : sql select success";
...
QString watchhistorylist::GetFindKind(int kind)
{
    QString thekind = "";
    if(kind == 0)
    {
...
void watchhistorylist::on_pushButton_clicked()
{
